...
def _plan(dist: Path, remote: str, branch: str, orig_branch: str) -> list:
    # 用 git 的 --work-tree 把 dist 當工作樹，提交到孤立分支再 push，最後還原分支。
    return [
...
def deploy_gh_pages(dist, remote: str = "origin", branch: str = "gh-pages",
    ...
    cmds = _plan(dist, remote, branch, orig_branch)
    if dry_run:
        return cmds
    for cmd in cmds:
        if log:
